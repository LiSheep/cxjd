...
<%
	...
	String basePath = request.getScheme() + "://"
			...
			+ path + "/";
	String actionName = "listOrganizer.action";			
%>
<html>
	<head>
		<title>承办单位管理</title>
		...
		<script type="text/javascript">	
			...
			function addEntity() {
				var addUrl = "url:<%=basePath%>inputOrganizer.action";
				var vDialog = $.dialog({
					id: 'addWin',
					title: '承办单位管理',
					lock: true,
			...
			function editEntity(entityKey) {
				var editUrl = "url:<%=basePath%>inputOrganizer.action?key=" + entityKey;
				var vDialog = $.dialog({
					id: 'editWin',
					title: '承办单位管理',
					lock: true,
			...
			function listEntity() {			  
				document.forms[0].action = "<%=basePath%>listOrganizer.action";			   
				document.forms[0].submit();	
			}
			
			function toResume(entityKey) {
				document.forms[0].action = "<%=basePath%>listResume.action?model.OrganizerKey="+entityKey;		   
				document.forms[0].submit();	
			}
		</script>
	</head>
											...
											<s:iterator value="entities">
												<tr>
													...
													<td width="120">
														...
														<a href='deleteOrganizer.action?key=<s:property value="id" />'>删除</a>
													</td>
												</tr>
											</s:iterator>
										</tbody>
