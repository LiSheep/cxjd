...
<%
	...
	String basePath = request.getScheme() + "://"
			...
			+ path + "/";
	String actionName = "listTool.action";			
%>
<html>
	<head>
		...
		<script type="text/javascript">	
			...
			function editEntity(entityKey) {
				var editUrl = "url:<%=basePath%>inputTool.action?key=" + entityKey;
				var vDialog = $.dialog({
					id: 'editWin',
					title: '工具管理',
					lock: true,
			...
			function applyTool(entityKey) {
				var editUrl = "url:<%=basePath%>applyBorrow.action?key=" + entityKey;
				var vDialog = $.dialog({
					id: 'editWin',
					title: '设备申请',
					lock: true,
			...
			function listEntity() {			  
				document.forms[0].action = "<%=basePath%>listTool.action";			   
				document.forms[0].submit();	
			}
			function listEntity2() {			  
										...
										<tbody>											
											<s:iterator value="entities">
												<tr>
													...
													<td><s:property value="ToolNum" /></td>
													<td><s:property value="ToolOut" /></td>
													<td><s:property value="%{getToolLeft(ToolNum, ToolOut)}" /></td>
													<td><s:property value="%{getPlace(ToolPlace)}" /></td>
													<td><s:property value="%{getUserByKey(UserKey)}" /></td>
													<td width="120">
														<s:if test="#session.role==1 || #session.role==3">
															<input type="button" class="I_btn_01" onclick="editEntity('<s:property value="id" />');" value="修改"/>
															<a href='deleteTool.action?key=<s:property value="id" />'>删除</a>
														</s:if>
														
														<s:if test= '#session.role==2'><!-- 学生 -->
															<input type="button" class="I_btn_01" onclick="applyTool('<s:property value="id" />');" value="申请"/>
														</s:if>
