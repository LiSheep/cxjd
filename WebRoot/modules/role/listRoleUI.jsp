...
<html>
	<head>
		...
		<script type="text/javascript">	
			...
			function addEntity() {
				var addUrl = "url:<%=basePath%>inputRole.action";
				var vDialog = $.dialog({
					id: 'addWin',
					title: '用户管理',
					lock: true,
			...
			function editEntity(entityKey) {
				var editUrl = "url:<%=basePath%>inputRole.action?key=" + entityKey;
				var vDialog = $.dialog({
					id: 'editWin',
					title: '用户管理',
					lock: true,
			...
			function listEntity() {			  
				document.forms[0].action = "<%=basePath%>listRole.action";			   
				document.forms[0].submit();	
			}
			
			function toResume(entityKey) {
				document.forms[0].action = "<%=basePath%>listResume.action?model.RoleKey="+entityKey;		   
				document.forms[0].submit();	
			}
		</script>
	</head>
										...
										<tbody>											
											<s:iterator value="entities">
												<tr>
													...
													<td><s:property value="%{getRoleByType(roleAuth)}" /></td>
													<td width="120">
														<input type="button" class="I_btn_01" onclick="editEntity('<s:property value="id" />');" value="修改"/>
														<a href='deleteRole.action?key=<s:property value="id" />'>删除</a>
													</td>
												</tr>
											</s:iterator>
										</tbody>
