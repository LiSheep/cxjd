...
<%
	...
	String basePath = request.getScheme() + "://"
			...
			+ path + "/";
	String actionName = "listSchoolNews.action";	
%>
<html>
	<head>
		<title>学校新闻<s:if test="#session.role==1 || #session.role==3">管理</s:if></title>
		...
		<script type="text/javascript">	
			...
			function listEntity() {			  
				document.forms[0].action = "<%=basePath%>listSchoolNews.action";			   
			...
			<div id="main" class="ks-clear">
				...
				<div id="mi_container">
					<div class="module_01">
						...
						<div class="btn_list">
							<input type="button" class="I_btn_02" onclick="window.location.href='<%=basePath%>inputSchoolNews.action'" value="添加"/>
						</div>
						</s:if>
						<div class="bd">
							<div class="info_list">
									...
									<table width="100%">
										...
										<tbody>											
											<s:iterator value="entities">
												<tr>
													<td><a href='detailSchoolNews.action?key=<s:property value="id" />'><s:property value="newsTitle" /></a></td>
													<td width="100"><s:date name="newsDate" format="yyyy-MM-dd" /></td>
													<s:if test="#session.role==1 || #session.role==3">
													<td width="120">
														<input type="button" class="I_btn_01" onclick="window.location.href='<%=basePath%>inputSchoolNews.action?key=<s:property value="id" />'" value="修改"/>
														<a href='deleteSchoolNews.action?key=<s:property value="id" />'>删除</a>
													</td>
													</s:if>
												</tr>
											</s:iterator>
